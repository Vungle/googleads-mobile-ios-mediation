...
- (nonnull instancetype)initWithAdConfiguration:(nonnull GADMediationBannerAdConfiguration*)adConfiguration
  ...
  if (self) {
    ...
    /// Unique ID is used to handle refreshes
    /// Every new instance of this class would be a new banner request
    self.uniquePubRequestID = [[NSUUID UUID] UUIDString];
...
- (void)requestBannerAd {
  ...
  if (!appID) {
    NSError *error = GADMAdapterVungleErrorWithCodeAndDescription(GADMAdapterVungleErrorInvalidServerParameters, @"Vungle app ID not specified.");
    _adLoadCompletionHandler(nil, error);
    return;
  }
...
- (NSError *)renderAd {
  UIViewController *viewController = _adConfiguration.topViewController;
